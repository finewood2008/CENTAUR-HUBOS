    ...
    <div className="flex-1 overflow-y-auto p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <motion.div
          ...
          className="bg-white/[0.03] rounded-xl border border-white/5 p-4 mb-5"
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-400">
                  已配置 <span className="text-white font-medium">{providers.length}</span> 个平台
                  {' · '}
        ...
        {providers.length === 0 && !showAddPanel && (
          ...
          >
            ...
            <p className="text-sm text-gray-400 mb-1">还没有配置任何平台</p>
            <p className="text-xs text-gray-600 mb-4">添加一个 AI 平台的 API Key 来开始使用</p>
            <button
        ...
        {providers.length > 0 && !showAddPanel && (
          <button
            onClick={() => setShowAddPanel(true)}
            className="w-full py-3 border border-dashed border-white/10 rounded-xl text-sm text-gray-500 hover:text-gray-300 hover:border-white/20 transition-colors flex items-center justify-center gap-2"
          >
            <Plus size={14} />
            添加平台
          </button>
        ...
        <AnimatePresence>
          {showAddPanel && (
            ...
            >
              <div className="bg-white/[0.03] rounded-xl border border-white/5 p-4 mt-3">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-medium text-white">选择平台</h3>
                  <button
                ...
                <div className="grid grid-cols-2 gap-2">
                  {availablePresets.map(([key, preset]) => (
                    <button
                      ...
                      className="flex items-center gap-3 p-3 bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 hover:border-white/10 rounded-lg transition-all text-left group"
                    >
                      <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${preset.color} flex items-center justify-center text-sm shrink-0`}>
                        {preset.icon}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm text-white group-hover:text-orange-300 transition-colors">{preset.name}</p>
                        <p className="text-[10px] text-gray-600 truncate">{preset.description}</p>
                      </div>
                    </button>
                  ))}
                </div>
...
function ProviderCard({
  ...
  const statusConfig = {
    untested: { color: 'text-gray-500', bg: 'bg-gray-500/10', label: '未测试' },
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <div className="flex items-center gap-2">
              ...
              {provider.isDefault && (
                <span className="text-[10px] px-1.5 py-0.5 bg-orange-500/15 text-orange-400 rounded">
                  默认
                </span>
              )}
            </div>
            ...
            <div className="px-4 pb-4 space-y-3 border-t border-white/5 pt-3">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="sk-..."
                    className="w-full px-3 py-2 pr-20 bg-white/5 border border-white/10 rounded-lg text-sm text-white font-mono placeholder-gray-600 focus:outline-none focus:border-orange-500/30"
                  />
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1.5 block">
                  可用模型
                  <span className="text-gray-600 ml-1">（逗号分隔）</span>
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white font-mono placeholder-gray-600 focus:outline-none focus:border-orange-500/30"
                />
              </div>
