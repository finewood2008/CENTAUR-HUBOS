...
export default function AgentManagement({ agents, templates }: AgentManagementProps) {
        ...
        <AnimatePresence mode="wait">
          {subView === 'market' && (
            <motion.div key="market" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <Market templates={templates} searchTerm={searchTerm} setSearchTerm={setSearchTerm} />
            </motion.div>
          )}
          {subView === 'roster' && (
...
// ─── 子视图：员工市场 ───
function Market({
  templates, searchTerm, setSearchTerm,
}: {
  templates: Template[];
  searchTerm: string;
        ...
        {filtered.map((tpl, i) => (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <button className="flex items-center gap-1 px-3 py-1.5 bg-orange-500/10 text-orange-400 text-xs rounded-lg hover:bg-orange-500/20 transition-colors border border-orange-500/20 opacity-0 group-hover:opacity-100">
                <Download size={12} /> 入职
              </button>
            </div>
          </motion.div>
  ...
  const send = () => {
    ...
    setTimeout(() => {
      const replies = [
        ...
        '完美！我已经整理好岗位说明书了：\n\n✅ 岗位名称已设定\n✅ 核心职责已明确\n✅ 模型已选择\n✅ 工具权限已配置\n\n接下来我会生成 SOUL.md 和 Harness 配置文件。确认后就可以"入职"了！',
      ];
      const idx = Math.min(messages.length - 1, replies.length - 1);
      setMessages((prev) => [...prev, { role: 'ai', content: replies[idx >= 0 ? idx : 0] }]);
    }, 800);
  };
    ...
    <div className="max-w-2xl mx-auto flex flex-col h-full">
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="描述你想招什么样的员工..."
          className="flex-1 px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-orange-500/30"
        />
        <button
          onClick={send}
