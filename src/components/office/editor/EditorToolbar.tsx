...
import type { ColorValue } from '../uiTypes';
import { CANVAS_FALLBACK_TILE_COLOR } from '../constants';
...
import { getWallSetCount, getWallSetPreviewSprite } from '../wallTiles';

/** Minimal Button shim – replace with Hub OS design-system button */
function Button(props: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: string; size?: string }) {
  const { variant: _v, size: _s, className, ...rest } = props;
  return <button className={className} {...rest} />;
}

/** Minimal ColorPicker shim – replace with Hub OS color picker */
function ColorPicker(_props: { value: ColorValue; onChange: (c: ColorValue) => void; colorize?: boolean; showColorizeToggle?: boolean }) {
  return <div>[ColorPicker placeholder]</div>;
}

/** Minimal ItemSelect shim – replace with Hub OS item select */
function ItemSelect(props: {
  width: number; height: number; selected: boolean;
  onClick: () => void; title: string; deps: unknown[];
  ...
  return (
    <button
      ...
      style={{
        ...
        border: props.selected ? '2px solid white' : '1px solid #555',
